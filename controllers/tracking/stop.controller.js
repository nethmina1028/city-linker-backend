...
const createTrackingStop = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to create TrackingStop", error: error.message });
  }
};
...
const updateTrackingStop = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update TrackingStop", error: error.message });
  }
};

const deleteTrackingStop = async (req, res) => {
  try {
    const deletedTrackingStop = await TrackingStopModel.findByIdAndDelete(
      req.params.id
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to delete TrackingStop", error: error.message });
  }
};
