...
const getTrackingPaths = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch TrackingPaths", error: error.message });
  }
};
...
const createTrackingPath = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to create TrackingPath", error: error.message });
  }
};
...
const updateTrackingPath = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update TrackingPath", error: error.message });
  }
};

const deleteTrackingPath = async (req, res) => {
  try {
    const deletedTrackingPath = await TrackingPathModel.findByIdAndDelete(
      req.params.id
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to delete TrackingPath", error: error.message });
  }
};
